...
1. **Đường chính — MCP connector** (chạy server-side, KHÔNG qua egress của container):
   `mcp__PubMed__search_articles`, `get_article_metadata`, `get_full_text_article`, `convert_article_ids`, `find_related_articles`; `mcp__Scite__search_literature` (kèm `editorialNotices` để phát hiện bài bị rút)
