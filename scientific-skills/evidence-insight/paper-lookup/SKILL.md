...
1. **Đường chính — MCP connector** (chạy server-side, KHÔNG qua egress của container):
   `mcp__PubMed__search_articles`, `get_article_metadata`, `get_full_text_article`, `convert_article_ids`, `find_related_articles`; `mcp__Scholar_Gateway__semanticSearch`, `mcp__Consensus__search`, `mcp__Elicit__search_papers`; `mcp__bioRxiv__search_preprints`, `get_preprint`, `search_published_preprints`
2. **Dự phòng — REST API trực tiếp**: chỉ dùng khi không có connector phù hợp, hoặc
   cần tham số mà connector không hỗ trợ.

Host REST mà skill này cần: `eutils.ncbi.nlm.nih.gov, api.crossref.org, api.openalex.org, api.semanticscholar.org, api.unpaywall.org`
