...
# Ký tự KHÔNG được nằm trong DOI khi rút từ văn bản. Thêm dấu nháy đơn/kép và ngoặc đóng:
# trong khối DATA của dashboard, DOI viết là  doi:'10.1002/alz.14333'  — bản trước nuốt luôn
# dấu nháy đóng, cho ra DOI hỏng. Scite tra không ra ⇒ bị xếp "CHƯA KIỂM", che mất việc
# thực chất là chưa kiểm được gì. "Không tra được" khác "không bị rút".
# DOI HỢP LỆ CÓ THỂ CHỨA NGOẶC: 10.1016/S0140-6736(24)01296-0 (Lancet, Elsevier).
# Vì vậy KHÔNG loại ')' trong lớp ký tự — làm thế sẽ cắt cụt DOI thật thành
# '10.1016/s0140-6736(24'. Chỉ loại dấu nháy (không bao giờ thuộc DOI), rồi cắt đuôi
# thừa bằng `got_doi` ở dưới, nơi ngoặc được đếm cân bằng.
DOI_RE = re.compile(r'''10\.\d{4,9}/[^\s"'<>,;\]}]+''', re.IGNORECASE)


def got_doi(raw: str) -> str:
    """Cắt đuôi thừa của một DOI rút từ văn bản chạy.

    Hai nguồn rác khác nhau, phải xử lý khác nhau:
      * Dấu câu cuối câu ('.', ',', ':') — cắt thẳng.
      * Ngoặc đóng ')' — CHỈ cắt khi thừa so với ngoặc mở, vì DOI của Elsevier
        có ngoặc cân bằng hợp lệ: 10.1016/S0140-6736(24)01296-0.
    """
    d = raw.rstrip(".,:;")
...
def unwrap_scite(obj, _depth=0):
    """Trả về đối tượng Scite dạng {"hits": [...]} từ mọi dạng vỏ có thể gặp.

    Agent gọi connector qua MCP thì kết quả KHÔNG phải JSON thô của Scite mà là phong bì:
        [{"type": "text", "text": "<chuỗi JSON>"}]
    Bản trước chỉ nhận JSON thô nên vỡ ngay với thứ mà thực tế nó nhận được
    (AttributeError: 'list' object has no attribute 'get'). Nay nhận cả ba dạng:
      1. {"hits": [...]}                      — JSON thô của Scite
      2. [{"type":"text","text":"{...}"}]     — phong bì MCP (bọc bao nhiêu lớp cũng gỡ)
      3. [ {...}, {...} ]                     — mảng hit trần
    """
    if _depth > 5:
        raise ValueError("lồng quá sâu")
    if isinstance(obj, str):
        return unwrap_scite(json.loads(obj), _depth + 1)
    if isinstance(obj, dict):
        ...
        if "doi" in obj:
            return {"hits": [obj]}
        raise ValueError("dict không có khoá 'hits'")
    if isinstance(obj, list):
        if not obj:
            return {"hits": []}
        # phong bì MCP: nhặt phần text rồi gỡ tiếp
        texts = [b["text"] for b in obj
                 if isinstance(b, dict) and b.get("type") == "text" and "text" in b]
        if texts:
            hits = []
            for t in texts:
                hits.extend(unwrap_scite(t, _depth + 1).get("hits", []))
            return {"hits": hits}
        if all(isinstance(x, dict) for x in obj):
            return {"hits": obj}
    raise ValueError("dạng không nhận ra: %s" % type(obj).__name__)
...
def main() -> int:
    ...
    try:
        scite = unwrap_scite(json.loads(raw))
    except json.JSONDecodeError as exc:
        print(f"JSON không hợp lệ: {exc}", file=sys.stderr)
        return 2
    except ValueError as exc:
        print(f"Không nhận ra dạng JSON của Scite: {exc}", file=sys.stderr)
        return 2
